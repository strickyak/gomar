...
#  go run --tags=coco1,noos,vdg,cocoio,trace  gomar.go -rom_8000 /tmp/rom1 -internal_rom_listing /tmp/list1 --cart ../build-frobio/axiom41.rom --show_vdg_screen=1 --bracket_terminal  -global_map ~/coco-shelf/nekotos/kernel/_kernel.map -global_listing ~/coco-shelf/nekotos/kernel/_kernel.o.list  -t=1
...
set -x
go run --tags=coco1,nekotos,vdg,cocoio,$TRACE  gomar.go \
    -rom_8000 /tmp/rom1 \
